...
- 右下角活动题目卡片可以单独关闭；关闭只隐藏卡片，不会删除题目、停止自动作答或影响提交，题目离开活动状态后关闭记录会自动清理。
...
- 1.21.5: 新增按课程隔离的自动作答队列持久化；刷新后可恢复已排队或被中断的题目；新增题目页、活动题目、题目列表和 AI 面板的 AI 强制作答及编辑答案提交；新增自动恢复、过期强制补交和未作答扫描三个独立开关，默认关闭；调整弹幕为“连续 7 条/30 秒”选择最高频文本（并列取最新）后自动跟发；修复历史时间线误触发新题提醒和自动作答；新增恢复状态、AI 执行器、答案编辑、弹幕与提醒边界测试
