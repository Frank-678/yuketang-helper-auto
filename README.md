...
- 设置中可分别选择是否提醒：新题/答题开始、新一轮弹幕开始、重复弹幕达到跟发条件、考试/测试题组发布、课件发布、其他发布、下课，以及自动作答的排队、开始、成功、失败；系统通知、页面弹窗和提示声音也都可以单独关闭。工具栏铃铛仍是总开关。
...
- 可选开启“重复弹幕自动跟发”：当前班级最近 7 条弹幕中，同一文本出现 3 次时自动跟发相同文本；相邻弹幕间隔达到 60 秒视为新一轮，每轮最多跟发 2 条，同一文本每轮只跟发一次。提醒设置还可以分别提示新一轮开始，以及同文弹幕达到第 3 次、即将跟发的时刻。需要课堂弹幕输入框可用。
