...
### 使用方式
构建后的当前产物是 `dist/ykt-helper-1216.user.js`；发布前将它与根目录的 `../release/ykt-helper-1216.user.js` 保持完全一致，再导入 Tampermonkey。旧的 1.21.5 及更早发布包保留作历史版本。

本版本修复不限时题目的 `limit` 为空时被错误判定为已过期的问题；空值、`0` 和非法时限均按“不限时”处理，正数时限仍按秒计算。

课堂互动还提供可选的“重复弹幕自动跟发”：连续 7 条弹幕在 30 秒内时，跟发出现次数最多的文本；7 条都不相同时跟发最新一条。相邻弹幕间隔达到 60 秒重新分轮，每轮最多 2 条，同一文本每轮只跟发一次，默认关闭。提醒设置可分别提示新一轮开始和达到跟发条件的时刻。

1.21.5 新增自动作答恢复：自动作答队列会按课程保存在本地，刷新后可恢复刷新前已经排队或正在 AI 处理的题目。恢复和扫描均为独立开关且默认关闭；已过截止时间的自动补交还需要单独开启。当前选中的题目页显示该题的“AI 强制作答”和“编辑/补交”；未选中题目页时仍通过题目列表进入，列表保留原有“强制补交”并增加“AI 强制作答”。右下角活动题目卡片不再显示题干或操作，避免重复入口。

1.21.6 增加了跨活跃课堂的实时连接与发布提醒上下文；发布提醒会跳过当前正在查看的课件，但保留其他课堂和同课堂其他课件的提醒。弹幕跟发需要实际出站帧确认；截止后强制补交、快速作答时间窗和双 Profile 复核均为可选设置；自动课堂跳转会等待 10 秒并尊重用户操作。

自动作答支持可选的截止后强制补交；还可以配置快速作答时间点、快速 Profile 和复核 Profile。快速答案会先提交，复核模型只有在返回不同答案时才执行一次纠正提交。

自动进入课堂会严格选择当前账号的 `status=1` 活跃课堂，并在自动导航前等待 10 秒；用户在等待期间进行操作后，脚本不再抢占导航。
...
│   ├── danmu-follow.js         # 弹幕提取、轮次检测、滑窗统计、分轮限额与自回显排除
│   ├── danmu-sender.js         # 复用课堂原生弹幕输入框和发送按钮
│   ├── realtime-dispatch.js    # 运行模式感知的实时事件分发
│   ├── problem-event-source.js  # 区分实时题目解锁与历史时间线回放
...
- `reminder-preferences.js` 是唯一的提醒开关定义：总开关、11 个事件开关和系统通知/页面弹窗/声音三种方式开关都由它管理。
...
- 统计当前班级连续 7 条非空弹幕；7 条消息的首尾时间差不超过 30 秒时，选择出现次数最多的精确文本，并通过课堂页面的 `.send__input` 和 `.send__btn` 自动发送。并列时选择最新一条消息。
- 相邻两条弹幕间隔 `< 60` 秒属于同一发送轮，间隔 `>= 60` 秒开始新轮；每轮最多跟发 2 条，同一文本每轮只跟发一次。
- 课堂提醒可以独立提示“新一轮弹幕开始”和“7 条弹幕达到跟发条件”；前者不依赖自动跟发开关，后者只在自动跟发实际达到条件时触发。历史时间线回放只恢复题目状态，不触发新题提醒或自动作答。两者都沿用总提醒开关和三种提醒方式设置。
- 功能默认关闭；发送框不存在、发送按钮禁用或发送失败时不会重试刷屏，也不会影响题目和课堂提醒。

### 题目操作入口

- 只有当前选中的课件题目页会显示该题的题干，以及“AI 强制作答”“编辑/补交”按钮。
- 不在题目页时，从“题目列表”点击“查看”进入对应页面，或直接使用列表中的“AI 强制作答”“强制补交”；原有手动编辑和提交流程保持不变。
- 右下角不再挂载活动题目卡片，避免在非当前页面重复显示题目内容和操作。
...
- 在当前题目页、题目列表或 AI 面板点击 **AI 强制作答**，会立即开始 AI 请求，不等待自动作答延迟；题目过期时会先确认，再调用 `/api/v3/lesson/problem/retry`。不在题目页时，先从题目列表点击“查看”进入对应页面，或直接使用列表中的“AI 强制作答/强制补交”。
...
- **problem-list**: 题目历史记录
- **active-problems**: 保留兼容的活动题目更新接口；不再在右下角重复渲染题目内容，题目操作由当前题目页和题目列表提供
- **tutorial**: 使用教程
