...
课堂互动还提供可选的“重复弹幕自动跟发”：最近 7 条弹幕中同一文本出现 3 次时跟发相同文本；相邻弹幕间隔达到 60 秒重新分轮，每轮最多 2 条，同一文本每轮只跟发一次，默认关闭。提醒设置可分别提示新一轮开始和达到跟发条件的时刻。
...
- 课堂提醒可以独立提示“新一轮弹幕开始”和“同一文本第 3 次出现、即将跟发”；前者不依赖自动跟发开关，后者只在自动跟发实际达到条件时触发。两者都沿用总提醒开关和三种提醒方式设置。
