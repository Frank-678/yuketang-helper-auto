...
- 雨课堂当前 PC 包会在竖屏桌面入口检测 `innerWidth < innerHeight` 后跳转 `/m/v2`。`runtime-mode.js` 只在 `/`、`/v2/web`、`/v2/web/index` 覆盖该宽度判定；不伪造 User-Agent，也不影响课堂页尺寸。
- 移动路径回跳检测保留 15 秒状态，桌面页不会立即清除它；若同一路径立刻回到 `/m/v2`，守卫会停在手机版并提示用户启用浏览器“桌面版网站”，而不会循环。
