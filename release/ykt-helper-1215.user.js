...
(function() {
 ...
 // ★ 标记为已完成
  ...
  /** Select the server route for one answer attempt. */  function chooseAnswerRoute({now: now = Date.now(), endTime: endTime = null, forceRetry: forceRetry = false} = {}) {
    if (forceRetry) return "retry";
    if (endTime === null || endTime === void 0 || endTime === "") return "answer";
    const current = Number(now);
    const deadline = Number(endTime);
    if (Number.isFinite(current) && Number.isFinite(deadline) && current >= deadline) return "retry";
    return "answer";
  }
  function sleep$1(ms) {
 ...
 // E6
   ...
   */  function createDanmuFollowTracker(options = {}) {
    ...
    function observe(text, at = Date.now()) {
      ...
      if (followedTexts.has(selectedText)) return result(false, selectedText, count, "already-followed");
   ...
   */  function createDanmuFollowController(options = {}) {
    ...
    async function handle(message, {notificationOnly: notificationOnly = false} = {}) {
      ...
      if (sendResult === true || sendResult?.sent === true && sendResult?.verified !== false) rememberOwn(observation.text, now);
      return {
        handled: true,
        ...observation,
 ...
 // 多选
  ...
  async function verifyAutoAnswer({problem: problem, imageBase64: imageBase64, textPrompt: textPrompt, firstAnswer: firstAnswer, firstRawAnswer: firstRawAnswer, status: status, forceRetry: forceRetry}) {
    if (ui.config.answerVerification !== true) return {
      answer: firstAnswer,
      rawAnswer: firstRawAnswer,
      state: "disabled"
    };
    ...
    const verificationPrompt = [ textPrompt, "", "【快速模型候选答案】", JSON.stringify(firstAnswer), "", "请使用当前课件图片和题干独立复核候选答案。不要盲从候选答案；如果候选答案错误，请给出你认为正确的答案。", "必须按原题型要求输出，并包含“答案:”字段；只输出最终答案和必要的简短解释。" ].join("\n");
    const verifiedRawAnswer = await queryAIVision(imageBase64, verificationPrompt, ui.config.ai, {
  ...
  // 融合模式自动答题
    async function handleAutoAnswerInternal(problem, {forceRetry: forceRetry = false} = {}) {
 ...
 // 提前返回，避免继续走图像+AI流程
      ...
      try {
        ...
        if (verificationState === "corrected") console.log("[雨课堂助手][INFO][AutoAnswer] 验证模型发现差异，已重新提交答案");
      ...
      status.done = true;
      status.answering = false;
      const successDetail = verificationState === "corrected" ? "快速答案已提交，验证模型发现错误并完成修正。" : verificationState === "confirmed" ? "答案已提交，并通过验证模型复核。" : verificationState === "unavailable" || verificationState === "invalid" ? "答案已提交，但验证模型暂未完成复核。" : void 0;
  ...
  const actions = {
    ...
    onUnlockProblem(data, {notificationOnly: notificationOnly = false, lessonId: lessonId = null} = {}) {
      const payload = data && typeof data === "object" ? data : {};
