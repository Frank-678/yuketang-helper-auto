 ...
 // E6
  ...
  function createAutoAnswerRunner({typeMap: typeMap = {}, hasActiveProfile: hasActiveProfile = () => false, getAIConfig: getAIConfig = () => {}, makeDefaultAnswer: makeDefaultAnswer = () => null, captureSlideImage: captureSlideImage = async () => null, captureProblemForVision: captureProblemForVision = async () => null, formatProblemForVision: formatProblemForVision = () => "", queryAIVision: queryAIVision = async () => "", parseAIAnswer: parseAIAnswer = () => null, submitAnswer: submitAnswer = async () => ({
    ...
    async function run(problem, status, {force: force = false, forceRetry: forceRetry = false, allowResubmit: allowResubmit = false, source: source = (force ? "manual" : "auto"), lessonId: lessonId = null} = {}) {
      ...
      try {
        let parsed;
        if (!hasActiveProfile(getAIConfig())) parsed = makeDefaultAnswer(problem); else {
          let image = null;
          try {
        ...
        notify?.("auto-answer-succeeded", problem, shouldRetry ? "答案已强制补交。" : void 0, {
          source: source,
 ...
 // 多选
  ...
  const actions = {
    ...
    onUnlockProblem(data, {notificationOnly: notificationOnly = false} = {}) {
      ...
      status.autoAnswerQueued = isFirstUnlock ? !!ui.config.autoAnswer : status.autoAnswerQueued !== false;
