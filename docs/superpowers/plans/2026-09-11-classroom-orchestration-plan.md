# 1.21.5 课堂编排 Implementation Plan
...
- Modify: `README.md`
- Modify: `changelog.md`
- Create/replace: `release/ykt-helper-1215.user.js`
- Test: existing `ykt-helper/test/*.test.js`

- [ ] **Step 1: Update version references**

Change all current `1.21.4`/`1214` references that describe the active release to `1.21.5`/`1215`; keep older release files as history. Update installation URLs to the current organization repository where appropriate.
...
Run: `npm run build`
Expected: Rollup exits with status 0 and writes `dist/ykt-helper-1215.user.js`.

- [ ] **Step 4: Synchronize and compare release output**

Run: `cp dist/ykt-helper-1215.user.js ../release/ykt-helper-1215.user.js && cmp -s dist/ykt-helper-1215.user.js ../release/ykt-helper-1215.user.js`
Expected: `cmp` exits 0; generated header contains `@version 1.21.5`.
...
```bash
git add userscript.meta.js rollup.config.mjs readme.md ../README.md ../changelog.md ../release/ykt-helper-1215.user.js
git commit -m "release: publish yukt helper 1.21.5"
```
